...
test("Bilder-Wait: 404 wird gemeldet, Lazy-Bild wird nachgeladen, kein Hänger", async ({
  ...
  // Das Lazy-Bild wurde auf eager gestellt und ist geladen.
  expect(haengen).toEqual([]);
  expect(
    await page.locator("#spaetes-bild").evaluate((b: HTMLImageElement) => ({
      fertig: b.complete,
      breite: b.naturalWidth,
    })),
  ).toEqual({ fertig: true, breite: expect.any(Number) });
