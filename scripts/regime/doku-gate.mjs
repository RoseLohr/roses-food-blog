 ...
 *
 *    Verboten sind die Anweisungen, nicht die Wörter: geprüft wird NUR
 *    innerhalb umzäunter Codeblöcke (```). Fließtext darf und soll weiter
 *    sagen, dass es certbot hier nicht gibt — genau das steht in README §2.
 *    Ein Codeblock unter einer Überschrift, die „historisch" enthält, ist
 ...
 *
 * Kalibrierung (A-36): `--selftest` führt für jede der drei Prüfungen einen
 * synthetischen Verstoß ein und bestätigt, dass sie ihn fängt — und für die
 * Ausnahmen (historischer Block, Bezeichner in `src/`), dass sie NICHT feuern.
 */
...
/**
 * Prüfung 1: verbotene Anleitung in einem Codeblock.
...
export function verboteneAnleitung(text) {
  ...
  text.split("\n").forEach((zeile, i) => {
    if (/^#{1,6}\s/.test(zeile)) unterHistorisch = /historisch/i.test(zeile);
    if (/^\s*```/.test(zeile)) {
      imBlock = !imBlock;
      return;
    }
    if (imBlock && !unterHistorisch && VERALTETE_ANLEITUNG.test(zeile)) treffer.push(i + 1);
  });
  return treffer;
}
...
export function pfadverweise(zeile) {
  ...
  for (const m of zeile.matchAll(/`([^`\s]+)`/g)) {
    ...
    if (!t) continue;
    raus.push({ text: m[1], pfad: t[1].replace(/[.,;:)]+$/, ""), bis: t[3] ?? t[2] ?? null });
  }
  return raus;
}
...
function main() {
  ...
  // Ein Pfad zählt auch dann als getroffen, wenn er eine EINDEUTIGE Abkürzung
  // ist: `audit/06` meint `audit/06-residual-risk-register.md`, und diese
  // Schreibweise steht unter anderem in der Verfassung. Mehrdeutig wäre sie
  // kein Verweis mehr — dann schlägt sie fehl.
  const trifft = (p) => fs.existsSync(p) || alle.filter((f) => f.startsWith(p)).length === 1;

  let gezaehlt = 0;
  for (const datei of alle) {
    const imAudit = datei.startsWith("audit/");
    const istQuelle = datei.startsWith("src/") && /\.(ts|tsx|css)$/.test(datei);
    if (!istMd(datei) && !istQuelle) continue;
    const text = fs.readFileSync(datei, "utf8");

    if (istMd(datei) && !imAudit) {
      for (const nr of verboteneAnleitung(text)) {
        melde("Veraltete Anleitung im Codeblock (A2/A3)", `${datei}:${nr}`, text.split("\n")[nr - 1].trim());
      }
    }

    if (!imAudit) {
      for (const nr of toteNummern(text, istQuelle)) {
        melde("Verweis auf eine Nummerierung, die es nicht gibt", `${datei}:${nr}`, text.split("\n")[nr - 1].trim());
      }
    }

    if (!istMd(datei)) continue;
    text.split("\n").forEach((zeile, i) => {
      for (const v of pfadverweise(zeile)) {
        // In `audit/` nur Verweise MIT Zeilennummer — siehe Kopf.
        if (imAudit && v.bis === null) continue;
        gezaehlt++;
        if (!trifft(v.pfad)) {
          melde("Pfadverweis ins Leere", `${datei}:${i + 1}`, v.text);
          continue;
        }
        if (v.bis !== null && fs.existsSync(v.pfad) && fs.statSync(v.pfad).isFile()) {
          const zeilen = fs.readFileSync(v.pfad, "utf8").split("\n").length;
          if (Number(v.bis) > zeilen) {
            melde("Zeilennummer jenseits des Dateiendes", `${datei}:${i + 1}`, `${v.text} (Datei hat ${zeilen} Zeilen)`);
          }
        }
      }
    });
...
if (process.argv.includes("--selftest")) {
  const faelle = [
    // Prüfung 1 — fängt den Verstoß …
    ["Anleitung im Codeblock", verboteneAnleitung("## Setup\n```\nsudo certbot --nginx\n```\n").length === 1],
    ...
    // … und nicht im Fließtext, der die Abwesenheit gerade feststellt.
